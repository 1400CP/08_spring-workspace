			...
			$("#btn2").click(function(){
				$.ajax({
					...
					success:function(data){
						...
						let $table = $("<table border='1' align='center'></table>");
						...
						/*
						$table.append($thead, $tbody); // append는 붙이라는 의미
						$table.appendTo("#result2"); // table을 붙여라
						*/
						$table.append($thead, $tbody)
							  .appendTo("#result2");
						
						/*
							만일 위의 반복문을 돌리지 않고 아래와 같이 돌리면 안 됨
							$table.append($thead)
								  .append(data)
							** 응답데이터 xml형식을 곧바로 table안에 넣으면 문제 발생한다.
							=> 요소들을 바로 html화 시킬 수 없음.
							=> 반복문을 돌려가면서 table화 시켜야 함.
						*/
